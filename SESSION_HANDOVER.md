...
| "Copying a link in another app does nothing, but pasting into Linksi works" | **Partly solved, best-effort.** In Chromium browsers it now works: Chromium exposes a link's href in the node's **extras** under `AccessibilityNodeInfo.targetUrl`, which `node.text` inspection cannot see, and the service finds it via `getWindows()` (not `rootInActiveWindow`). Verified on the OPPO - the bubble service started four times as the owner copied links. **But Chromium populates those extras inconsistently**: the same gesture on the same browser gave `targetUrl nodes=9` one minute and `no targetUrl extras found` the next. So detection is best-effort, the clipboard is refused to a background reader, and share/paste stay the reliable routes. Read `TEST_REPORT.md` §51 **before** §50 - §50 wrongly concluded this was impossible |
