...
| "Copying a link in another app does nothing, but pasting into Linksi works" | **This is a platform limit, not a bug to chase.** All four channels are closed on the owner's OPPO/Android 16, each measured: the event carries no link; the clipboard is refused to a background reader (`clipboard probe at connect: readable=false`); a selection event is not reliably delivered; and the clicked node's tree contains no link. See `TEST_REPORT.md` §50 before spending a single round on this again. The paths that DO work are share, paste, and copy affordances that put the URL in the event |
